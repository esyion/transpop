...
export function TranslationResult({
  ...
  if (loading) {
    return (
      <Card
        key="loading"
        aria-live="polite"
        className="result-card result-state animate-in fade-in zoom-in-95 border-border/70 duration-300"
      >
  ...
  if (apiKeyMissing) {
    return (
      <Card
        key="missing-key"
        className="result-card result-state animate-in fade-in zoom-in-95 border-primary/25 duration-300"
      >
        <CardHeader className="pb-2">
          <CardTitle>请先添加 API 密钥</CardTitle>
          <CardDescription>
  ...
  if (error) {
    return (
      <Card
        key="error"
        role="alert"
        className="result-card result-state animate-in fade-in zoom-in-95 border-destructive/25 duration-300"
      >
        <CardHeader className="pb-2">
          <CardTitle className="text-destructive">翻译失败</CardTitle>
  ...
  if (!resultText) {
    return (
      <Card
        key="empty"
        className="result-card result-state empty-result animate-in fade-in border-border/70 duration-300"
      >
  ...
  return (
    <Card
      key="result"
      aria-live="polite"
      className="result-card result-state animate-in fade-in slide-in-from-bottom-2 border-border/70 duration-300"
    >
      <CardHeader className="flex-row items-start justify-between gap-4 pb-3">
        <div className="space-y-1">
          <CardTitle className="result-title text-foreground">
            翻译结果
          </CardTitle>
        </div>
        <Button
